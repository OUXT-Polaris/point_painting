  ...
  const sensor_msgs::msg::CameraInfo & camera_info)
{
  ...
  for (const auto & seg_info : seg_msg.segmentations) {
    ...
    for (const auto & img_point : seg_info.polygons[0].points) {
      seg_polygon.outer().emplace_back(
        bg::make<bg::model::d2::point_xy<int>>(img_point.x, img_point.y));
    }
    ...
    *iter_scores = 0.0;
    if (
      ...
      for (int i = 0; i < static_cast<int>(polygons.size()); ++i) {
        ...
        bool is_covered =
          boost::geometry::covered_by(img_point(img_point_x, img_point_y), polygons[i]);
        if (is_covered) {
